...
def unwrap_conv_tensor(x, filter_shape, n_samples):
    ...
    r = range(len(inds))
    try:
        s = np.random.choice(r, n_samples, replace=False)
    except:
        s = r
    sampled_inds = [inds[i] for i in s]
    # extract patches from sampled ims
    for (e, i, j) in sampled_inds:
        example = x[e]
        patch = example[i:i+fh, j:j+fw, :]
        patch_unwrapped = np.reshape(patch, (1, -1))
        unwrapped.append(patch_unwrapped)
    out = np.concatenate(unwrapped)
    return out
...
def initialize_network_weights(layers, init_type, sess, batch_func, batch_pl):
    ...
    for (W, b, pre_act, post_act, s, layer_input) in layers:
        ...
        if len(x.shape) == 4:
            ...
            conv = True
            x = unwrap_conv_tensor(x, (fh, fw), 10 * out_features)
            print("Unwrapped Shape", x.shape)
        else:
...
def C_tilde(fake_loss, W, b):
    ...
    # Track this variable as training progresses
    W_name = W.name.split('/')[-2]
    tf.scalar_summary("layer_learning_rate/"+W_name, C)
...
def weight_rescale_op(layers, alpha=.25):
    ...
    C_tilde_logs = [tf.log(c) for c in C_tildes]
    C_tilde_logs_mean = tf.add_n(C_tilde_logs) / len(C_tilde_logs)
    C = np.e**C_tilde_logs_mean
    rks = [(Ck/C)**(alpha/2.0) for Ck in C_tildes]

    W_assign_ops, b_assign_ops, s_assign_ops = [], [], []
    for (W, b, pre_act, post_act, s, layer_input), rk in zip(layers, rks):
        W_assign_ops.append(W.assign(rk * W))
        b_assign_ops.append(b.assign(rk * b))
        s_assign_ops.append(s.assign(tf.div(s, rk)))
    with tf.control_dependencies(W_assign_ops + b_assign_ops + s_assign_ops):
        weight_rescale_op = tf.no_op("weight_rescale")
    return weight_rescale_op

def between_layer_init(layers, sess, batch_func, batch_pl, iters=10, alpha=.25, loss=None):
    """
    ...
    if loss is None:
        print("Creating fake loss")
        last_pre_act = layers[-1][2]
        fake_target = tf.random_normal(tf.shape(last_pre_act))
        loss = tf.reduce_mean(tf.reduce_sum(fake_target * last_pre_act, [1]))


    # get all C_tiles (this is the expected learning rate per layer)
    C_tildes = [C_tilde(loss, W, b) for W, b, pre_act, post_act, s, layer_input in layers]
    ...
    for i in range(iters):
        fd = {bpl: b for bpl, b in zip(batch_pl, batch_func())}
        # evaluate C_tildes
        ct_values = sess.run(C_tildes, feed_dict=fd)
        print("Ct values: {}".format(ct_values))
        sess.run(weight_rescale_op, feed_dict=fd)
    ...
    scales = [p[4] for p in layers]
    scale_values = [s[0] for s in sess.run(scales)]
    print("Final scales: {}".format(scale_values))
    return weight_rescale_op

def magic_init(layers, init_type, sess, batch_func, init_batch_pl, loss=None, rescale_only=False):
    """
    ...
    op = between_layer_init(layers, sess, batch_func, init_batch_pl, loss=loss)
    return op
...
if __name__ == "__main__":
    ...
    weight_rescale_op = magic_init(layers, "kmeans", sess, batch_func, [x_pl])
    summary_op = tf.merge_all_summaries()

    with tf.control_dependencies([train_step]):
        with tf.control_dependencies([weight_rescale_op]):
